...
1如果有时间的话可以用lora微调mlp看看效果

2论文中要提到的：1参数轻量化，跟baseline对比参数；强化学习代价小，短epoch，多长度通用；

3论文里尝试说明在强化学习中lora相对于prefix的优越性
...
10仅仅用残差流来进行长文本生成，结合强化学习模块进行长文本生成，第一种情况一定会属性下降，第二种情况不会，说明rl的必要性，要把这里的rl描述的高大上（思考怎么说），毕竟训练代价很小

11展望：还有什么可以提高的点？画饼，我们会在其他更大的模型上采用该机制
...
行文思路：
Abstract
受控属性生成->短文本效果好->长文本属性下降->如果要维持会增大控制强度导致属性崩溃->我们的方法不会导致属性崩溃，还能稳定acc，简要介绍我们的方法
补充图1：选全量微调和部分微调和decoding-time的方法和我们的对比，说明长文本下我们方法的优越性
...
Abolation:
1只使用残差流，计算长文本的acc
2只使用gate,计算不同长度文本的acc
3只使用fuse_net，计算不同长度文本的acc
