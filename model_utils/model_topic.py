...
class RobertaForPreTraining(RobertaPreTrainedModel):
    ...
    def forward(self, input_ids, attention_mask=None):
        ...
        # 2. 运行分类头
        # prediction_scores 是 MLM 任务的输出 (维度: batch_size, seq_len, vocab_size)，这里我们不需要它
        # seq_relationship_score 是 Topic 任务的 logits (维度: batch_size, 4)
        _, seq_relationship_score = self.cls(sequence_output, pooled_output)

        # 3. 将 logits 转换为概率
        # dim=1 表示沿着类别维度进行归一化，使得每行概率之和为 1
        probs = F.softmax(seq_relationship_score, dim=1)

        # 返回概率矩阵 (shape: [batch_size, 4])
        return probs
