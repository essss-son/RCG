...
from transformers import GPT2ForSequenceClassification, RobertaTokenizer, pipeline
...
def main():
    model_path = "/home/anke/DXZ/models/distilgpt2"
    ...
    dataset_path = [
        "/home/anke/DXZ/RCG/version_9/dataset/sentiment-imdb/imdb_5k_pos_tokenized.json",
        "/home/anke/DXZ/RCG/version_9/dataset/sentiment-imdb/imdb_5k_neg_tokenized.json"
    ...
    for data_enum,dataset in enumerate(dataset_path):
        ...
        #
        # return
        #Total number of parameters: 82060032, Trainable parameters: 147456, Percentage: 0.18%
        ...
        for epoch in trange(train_epoch_for_each_dataset):
            ...
            model.eval()
            acc = evaluate(model,tokenizer,writer_dict[data_enum],task,epoch)
            # if acc > min_acc:
            #     min_acc = acc
...
def evaluate(model, tokenizer,attr,task,epoch=None):
    prompt_paths = {
    ...
    eval_file = lora_eval_file + f"/len{gen_length}_acc{ave_acc}_ppl{round(ave_ppl,2)}.txt"
...
def t(tokenizer,attr):
    def topic_paddding(lst):
        attr_dict = {
            "world":0,
            "sports":1,
            "business":2,
        ...
        data_list = [{"text":tokenizer.encode(text, max_length=512, truncation=True),"topic":attr_dict[attr]} for text in lst]
        ...
        batch["topic"] = topic
        return batch
    return topic_paddding


@torch.no_grad()
def eval_topic(texts,device,attr):
    model = RobertaForPreTraining.from_pretrained("/home/anke/DXZ/CTG_papers_code/Air-Decoding-main/models/best_topic_classifier").to(device)
    tokenizer = RobertaTokenizer.from_pretrained("/home/anke/DXZ/CTG_papers_code/Air-Decoding-main/models/best_topic_classifier")
    eval_batch = 16
    data = my_evalset(texts)
    loader = DataLoader(data, batch_size=eval_batch,shuffle=False, drop_last=False,collate_fn=t(tokenizer,attr))
    correct = 0
    for batch in loader:
        batch_topic = batch["topic"]
        input_ids_list = [torch.tensor(item, dtype=torch.int).to(device) for item in batch["input_ids"]]
        input_ids = torch.stack(input_ids_list, dim=0)
        attention_masks_list = [torch.tensor(item, dtype=torch.int).to(device) for item in batch["attention_mask"]]
        attention_mask = torch.stack(attention_masks_list, dim=0).to(torch.float)
        attention_mask = attention_mask.unsqueeze(1).unsqueeze(2)
        attention_mask = (1.0 - attention_mask) * -10000.0
        output = model(input_ids=input_ids, attention_mask=attention_mask).argmax(1).tolist()
        c_list = [1 if p[0] == t else 0 for p, t in zip(batch_topic, output)]
        correct += sum(c_list)
        # print(c_list)
