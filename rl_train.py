...
import numpy as np
import torch
from gen_eval.utils import compute_distinct
from utils.Agent import Agent
from tqdm import tqdm

from gen_eval.utils import my_eval_set
...
if __name__ == '__main__':
    ...
    parser.add_argument("--task", type=str, default="multi", help="sentiment ot topic or detoxification or multi")
    parser.add_argument("--attr", nargs="+", default=["pos","science"],help="pos neg or world sports business science or nontoxic")
    ...
    #note reward
    args.reward_scaling = 1.5
    args.base_probs_scaling = 0.08
    args.repeat_scaling = 0.0

    # note evaluate
    args.epochs_for_eval = 10
    args.num_sequence = num_s_dict[args.task]     #for each task  8 8 1 8
    args.collect_batch_size = collect_b_dict[args.task]  # for each task 8 8 1 8
    args.generate_length = 64
    args.eval_batch_size = 32


    # note record
    writer = SummaryWriter(log_dir=f"./rl_train/{args.task}/{"_".join(args.attr)}/" + args.version + "/logs/")
    args.writer = writer
    hyp_path = os.path.join(f"./rl_train/{args.task}/{"_".join(args.attr)}", args.version)
